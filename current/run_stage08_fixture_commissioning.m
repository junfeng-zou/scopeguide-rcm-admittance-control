function [summary, outputDirectory] = ...
        run_stage08_fixture_commissioning(options)
%RUN_STAGE08_FIXTURE_COMMISSIONING Parallel low-speed physical Stage 8.
% The worker enables the robot only after every typed/boolean commissioning
% gate passes. The operator must verify the independent emergency stop and
% fixture before authorizing motion. Cleanup disables the robot.
...
if ~options.ArmPhysicalMotion
    error('scopeguide:stage08:PhysicalMotionNotArmed', ...
        ['Stage 8 defaults to OFF. Set ArmPhysicalMotion=true only after ' ...
...
fprintf(['Stage 8 PHYSICAL FIXTURE commissioning: DOF=%s; ' ...
...
fprintf(['Soft/hybrid RCM recovery shaping: deadzone=%.2f mm; ' ...
    'smooth activation to %.2f mm. Hard RCM is unchanged.\n'], ...
    1e3 * cfg.qp.RcmRecoveryDeadzoneM, ...
    1e3 * cfg.qp.RcmRecoveryFullActivationM);
...
if string(future.State) ~= "finished"
    ...
    error('scopeguide:stage08:WorkerStopTimeout', ...
        ['Stage 8 worker did not stop within %.1f s and was cancelled. ' ...
...
fprintf('Stage 8 stopped: %s. ServoJ sent=%d, failures=%d.\n', ...
...
if options.KeepFigureOpenAfterRun && dashboard.isOpen()
    ...
    fprintf(['Final Stage 8 figure retained for manual review; ' ...
...
if any(~isfinite(values)) || any(values <= 0)
    error('scopeguide:stage08:InvalidOptions', ...
        'Stage 8 timing values must be finite and positive.');
end
if options.InputHeartbeatRateHz < 20
    error('scopeguide:stage08:InputHeartbeatTooSlow', ...
        'Stage 8 input heartbeat must be at least 20 Hz.');
end
end
