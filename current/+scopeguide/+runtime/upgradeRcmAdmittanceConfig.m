...
% the RCM wrench-shift, ServoJ step-guard, Stage 8 pivot-speed and soft-RCM
% recovery-shaping updates.  It does not change wrench thresholds, RCM
% coordinates or acceptance records.
...
if ~isfield(cfg, 'qp') || ~isstruct(cfg.qp)
    error('scopeguide:config:MissingQpField', ...
        'Legacy configuration is missing cfg.qp.');
end
if ~isfield(cfg.qp, 'RcmRecoveryDeadzoneM')
    cfg.qp.RcmRecoveryDeadzoneM = 0.00025;
end
if ~isfield(cfg.qp, 'RcmRecoveryFullActivationM')
