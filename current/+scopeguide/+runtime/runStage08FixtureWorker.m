...
cartesianWorker = isCartesianWorker(options);
...
if ~authorization.CommissioningAllowed
    error('scopeguide:runtime:WorkerAuthorizationBlocked', ...
...
if ~warmup.ReadyForTimingAudit
    error('scopeguide:runtime:QpWarmupFailed', ...
...
if options.WriteResults
    outputDirectory = createOutputDirectory(projectRoot, ...
        selectedDofMode(cfg, cartesianWorker), cartesianWorker);
    if cartesianWorker
        [logId, logCleanup] = openCartesianCommandLog( ...
            fullfile(outputDirectory, ...
            'cartesian_drag_control.csv')); %#ok<ASGLU>
    else
        [logId, logCleanup] = openCommandLog( ...
            fullfile(outputDirectory, 'stage08_control.csv')); %#ok<ASGLU>
    end
else
    outputDirectory = "";
    logId = -1;
...
relativePivotNormRad = nan(capacity, 1);
relativeInsertionAbsM = nan(capacity, 1);
relativeTravelRecoveryActive = false(capacity, 1);
qpStatusCodes = strings(capacity, 1);
relativeTranslationNormM = nan(capacity, 1);
relativeRotationNormRad = nan(capacity, 1);
...
while toc(clock) < options.DurationSec
    ...
    if afterReadSec >= nextControlSec
        ...
        if cartesianWorker && isfield(decision, 'RelativeCoordinate')
            relativeTranslationNormM(controlCount) = ...
                norm(decision.RelativeCoordinate(1:3));
            relativeRotationNormRad(controlCount) = ...
                norm(decision.RelativeCoordinate(4:6));
        elseif decision.GeometryValid
            rcmErrorM(controlCount) = decision.Geometry.RcmErrorNormM;
        end
        if ~cartesianWorker && ...
                isfield(decision.Qp, 'AppliedRelativeCoordinate')
            relativeCoordinate = double( ...
                decision.Qp.AppliedRelativeCoordinate(:));
            if numel(relativeCoordinate) == 3 && ...
                    all(isfinite(relativeCoordinate))
                relativePivotNormRad(controlCount) = ...
                    norm(relativeCoordinate(1:2));
                relativeInsertionAbsM(controlCount) = ...
                    abs(relativeCoordinate(3));
            end
        end
        if ~cartesianWorker && isfield(decision.Qp, ...
                'RelativeTravelRecoveryActive')
            relativeTravelRecoveryActive(controlCount) = logical( ...
                decision.Qp.RelativeTravelRecoveryActive);
        end
        qpStatusCodes(controlCount) = string(decision.Qp.StatusCode);
        ...
        if logId >= 0
            if cartesianWorker
                writeCartesianLogRow(logId, afterReadSec, frame, ...
                    decision, sendDuration, currentTrackingError, ...
                    uiAgeSec);
            else
    ...
    if afterReadSec >= nextDisplaySec && ~isempty(lastDecision)
        if cartesianWorker
            snapshot = ...
                scopeguide.runtime.buildCartesianDragDisplaySnapshot( ...
        ...
        snapshot.DofMode = selectedDofMode(cfg, cartesianWorker);
...
summary.Stage = 8;
if cartesianWorker
    summary.Stage = "cartesian_admittance_drag_no_rcm";
end
...
summary.ControllerFamily = "rcm_stage08";
summary.NoRcmConstraint = false;
summary.DofMode = selectedDofMode(cfg, cartesianWorker);
if cartesianWorker
    summary.ControllerFamily = "cartesian_drag";
    summary.NoRcmConstraint = true;
    summary.DofMask = scopeguide.control.cartesianDragDofMask( ...
        cfg.cartesianDrag.DofMode);
else
    summary.DofMask = scopeguide.control.stage08DofMask( ...
        cfg.stage08.DofMode);
end
...
summary.RelativePivotNormRad = statistics( ...
    relativePivotNormRad(indices));
summary.RelativeInsertionAbsM = statistics( ...
    relativeInsertionAbsM(indices));
summary.RelativeTravelRecoverySampleCount = sum( ...
    relativeTravelRecoveryActive(indices));
summary.ObservedQpStatusCodes = compactCodes( ...
    qpStatusCodes(indices), true);
summary.RelativeTranslationNormM = statistics( ...
    relativeTranslationNormM(indices));
summary.RelativeRotationNormRad = statistics( ...
    relativeRotationNormRad(indices));
...
summary.RequiresManualRcmMeasurementReview = ~cartesianWorker;
...
if options.WriteResults
    ...
    if cartesianWorker
        save(fullfile(outputDirectory, ...
            'cartesian_drag_worker_result.mat'), ...
            'summary', 'cfg', 'authorization');
    else
        save(fullfile(outputDirectory, 'stage08_worker_result.mat'), ...
            'summary', 'cfg', 'authorization');
    end
end
sendMessage(displayQueue, lifecycle( ...
...
function directory = createOutputDirectory(projectRoot, dofMode, cartesian)
timestamp = string(datetime('now', 'Format', 'yyyyMMdd_HHmmss_SSS'));
if cartesian
    prefix = "cartesian_drag_no_rcm_";
else
    prefix = "stage08_fixture_";
end
directory = fullfile(projectRoot, 'results', ...
    prefix + dofMode + "_" + timestamp);
...
function [fileId, cleanup] = openCartesianCommandLog(path)
[fileId, message] = fopen(path, 'w');
if fileId < 0
    error('scopeguide:cartesianDrag:CannotOpenLog', ...
        'Cannot open %s: %s', path, message);
end
cleanup = onCleanup(@() fclose(fileId));
names = ["timeSec", "dtSec", "sensorSeq", "robotSeq", ...
    "uiAgeSec", "fsm", "fault", "commandEligible", "servoSent", ...
    "servoSendSec", "trackingErrorRad", ...
    "forceQualityStatus", "wrenchSafetyStopCode", ...
    "forceX", "forceY", "forceZ", ...
    "momentX", "momentY", "momentZ", ...
    "desiredVx", "desiredVy", "desiredVz", ...
    "desiredWx", "desiredWy", "desiredWz", ...
    "achievedVx", "achievedVy", "achievedVz", ...
    "achievedWx", "achievedWy", "achievedWz", ...
    "relativeX", "relativeY", "relativeZ", ...
    "relativeRx", "relativeRy", "relativeRz", ...
...
function writeCartesianLogRow(fileId, timeSec, frame, decision, ...
        sendSec, trackingError, uiAgeSec)
wrench = zeros(6, 1);
if isfield(decision.Admittance, 'Mapping') && ...
        ~isempty(decision.Admittance.Mapping)
    wrench = decision.Admittance.Mapping.WrenchDragAtDragPoint(:);
end
fprintf(fileId, ['%.9g,%.9g,%.0f,%.0f,%.9g,%s,%s,%.0f,%.0f,' ...
    '%.9g,%.9g,%s,%s'], timeSec, decision.DtSec, ...
    ...
    double(decision.CommandEligible), double(decision.MotionCommandSent), ...
    sendSec, trackingError, ...
    sanitize(decision.Safety.ForceStatusCode), ...
    sanitize(decision.Safety.WrenchSafetyStopCode));
numeric = [wrench; decision.Admittance.DesiredTwistBase(:); ...
    decision.Qp.AchievedTwistBase(:); decision.RelativeCoordinate(:); ...
...
function tf = isCartesianWorker(options)
tf = isfield(options, 'ControllerFamily') && ...
    string(options.ControllerFamily) == "cartesian_drag";
end

function mode = selectedDofMode(cfg, cartesian)
if cartesian
    mode = string(cfg.cartesianDrag.DofMode);
else
