...
softPivotLimit = cfg.control.RelativePivotLimitRad;
softInsertionLimit = cfg.control.RelativeInsertionLimitM;
pivotRecoveryTolerance = ...
    cfg.control.RelativePivotRecoveryToleranceRad;
insertionRecoveryTolerance = ...
    cfg.control.RelativeInsertionRecoveryToleranceM;
hardPivotLimit = softPivotLimit + pivotRecoveryTolerance;
hardInsertionLimit = softInsertionLimit + ...
    insertionRecoveryTolerance;
pivotRecoveryActive = norm(relative(1:2)) > softPivotLimit;
insertionRecoveryActive = abs(relative(3)) > softInsertionLimit;
solution.SoftRelativePivotLimitRad = softPivotLimit;
solution.SoftRelativeInsertionLimitM = softInsertionLimit;
solution.RelativePivotRecoveryToleranceRad = ...
    pivotRecoveryTolerance;
solution.RelativeInsertionRecoveryToleranceM = ...
    insertionRecoveryTolerance;
solution.HardRelativePivotLimitRad = hardPivotLimit;
solution.HardRelativeInsertionLimitM = hardInsertionLimit;
solution.PivotTravelRecoveryActive = pivotRecoveryActive;
solution.InsertionTravelRecoveryActive = insertionRecoveryActive;
solution.RelativeTravelRecoveryActive = ...
    pivotRecoveryActive || insertionRecoveryActive;
if norm(relative(1:2)) > hardPivotLimit + ...
        cfg.qp.ConstraintTolerance || ...
        abs(relative(3)) > hardInsertionLimit + ...
        cfg.qp.ConstraintTolerance
...
[recoveryError2M, recoveryActivationScale] = ...
    shapeRcmRecoveryError(geometry.RcmError2M, cfg.rcm.Mode, cfg);
recoveryTarget = singularityScale * recoveryGain * ...
    recoveryError2M;
solution.RcmRecoveryDeadzoneM = cfg.qp.RcmRecoveryDeadzoneM;
solution.RcmRecoveryFullActivationM = ...
    cfg.qp.RcmRecoveryFullActivationM;
solution.RcmRecoveryActivationScale = recoveryActivationScale;
solution.RcmRecoveryEffectiveError2M = recoveryError2M;
solution.RcmRecoveryTarget2MSec = recoveryTarget;
...
solution.FailureCode = "";
if solution.RelativeTravelRecoveryActive
    solution.StatusCode = "QP_SOLVED_TRAVEL_RECOVERY_RCM";
else
...
solution.RcmRecoveryFullActivationM = ...
    cfg.qp.RcmRecoveryFullActivationM;
solution.RcmRecoveryActivationScale = NaN;
solution.RcmRecoveryEffectiveError2M = nan(2, 1);
solution.RcmRecoveryTarget2MSec = nan(2, 1);
solution.SoftRelativePivotLimitRad = ...
    cfg.control.RelativePivotLimitRad;
solution.SoftRelativeInsertionLimitM = ...
    cfg.control.RelativeInsertionLimitM;
solution.RelativePivotRecoveryToleranceRad = ...
    cfg.control.RelativePivotRecoveryToleranceRad;
solution.RelativeInsertionRecoveryToleranceM = ...
    cfg.control.RelativeInsertionRecoveryToleranceM;
solution.HardRelativePivotLimitRad = ...
    cfg.control.RelativePivotLimitRad + ...
    cfg.control.RelativePivotRecoveryToleranceRad;
solution.HardRelativeInsertionLimitM = ...
    cfg.control.RelativeInsertionLimitM + ...
    cfg.control.RelativeInsertionRecoveryToleranceM;
solution.PivotTravelRecoveryActive = false;
solution.InsertionTravelRecoveryActive = false;
solution.RelativeTravelRecoveryActive = false;
...
function [effectiveError2M, activationScale] = ...
        shapeRcmRecoveryError(error2M, mode, cfg)
% Keep strict hard-RCM equality recovery unchanged.  Soft and hybrid modes
% ignore sub-deadzone error and use a C1-continuous smoothstep ramp before
% restoring the original recovery target at the full-activation radius.
mode = string(mode);
if mode == "hard"
    activationScale = 1;
    effectiveError2M = error2M;
    return;
end

errorNormM = norm(error2M);
deadzoneM = cfg.qp.RcmRecoveryDeadzoneM;
fullActivationM = cfg.qp.RcmRecoveryFullActivationM;
if errorNormM <= deadzoneM
    activationScale = 0;
elseif errorNormM >= fullActivationM
    activationScale = 1;
else
    normalized = (errorNormM - deadzoneM) / ...
        (fullActivationM - deadzoneM);
    activationScale = normalized^2 * (3 - 2 * normalized);
end
effectiveError2M = activationScale * error2M;
end
...
% Once the accumulated pivot coordinate has entered the recovery band,
% replace every already-violated soft face with a zero outward-rate bound.
% The remaining soft faces and the outer hard polygon still constrain the
% motion, while velocity back toward the normal range stays feasible.
pivotRecoveryActive = norm(relative(1:2)) > ...
    cfg.control.RelativePivotLimitRad;
pivotRecoveryRows = 0;
if pivotRecoveryActive
    violatedSoftFaces = pivotB < 0;
    pivotB(violatedSoftFaces) = 0;
    [hardPivotNormals, hardPivotRadius] = inscribedPolygon( ...
        cfg.qp.RelativePivotPolygonSides, ...
        cfg.control.RelativePivotLimitRad + ...
        cfg.control.RelativePivotRecoveryToleranceRad);
    hardPivotA = dtSec * hardPivotNormals' * pivotJacobian;
    hardPivotB = hardPivotRadius - ...
        hardPivotNormals' * relative(1:2);
    pivotA = [pivotA; hardPivotA];
    pivotB = [pivotB; hardPivotB];
    pivotRecoveryRows = size(hardPivotA, 1);
end
...
insertionRecoveryActive = abs(relative(3)) > ...
    cfg.control.RelativeInsertionLimitM;
insertionRecoveryRows = 0;
if insertionRecoveryActive
    if relative(3) > cfg.control.RelativeInsertionLimitM
        insertionB(1) = 0;
    else
        insertionB(2) = 0;
    end
    hardInsertionLimit = cfg.control.RelativeInsertionLimitM + ...
        cfg.control.RelativeInsertionRecoveryToleranceM;
    hardInsertionA = insertionA;
    hardInsertionB = [hardInsertionLimit - relative(3); ...
        hardInsertionLimit + relative(3)];
    insertionA = [insertionA; hardInsertionA];
    insertionB = [insertionB; hardInsertionB];
    insertionRecoveryRows = size(hardInsertionA, 1);
end
...
groups = struct('RcmRows', size(rcmA, 1), ...
    ...
    'PivotRecoveryRows', pivotRecoveryRows, ...
    'InsertionRecoveryRows', insertionRecoveryRows);
end
...
hardRelativeViolation = max([ ...
    norm(predictedRelative(1:2)) - ...
        (cfg.control.RelativePivotLimitRad + ...
         cfg.control.RelativePivotRecoveryToleranceRad); ...
    abs(predictedRelative(3)) - ...
        (cfg.control.RelativeInsertionLimitM + ...
         cfg.control.RelativeInsertionRecoveryToleranceM); 0]);
...
diagnostics.MaximumSoftRelativeTravelExcess = softRelativeExcess;
% Preserve the historical field name, but make its fault-bearing meaning
% refer to the new outer hard boundary rather than the normal soft limit.
diagnostics.MaximumRelativeTravelViolation = hardRelativeViolation;
diagnostics.MaximumHardRelativeTravelViolation = hardRelativeViolation;
...
diagnostics.MaximumViolation = max([inequalityViolation; ...
    ...
    positionViolation; hardRelativeViolation; rcmViolation]);
