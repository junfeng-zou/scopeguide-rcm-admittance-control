...
| Pivot 时间常数 | 0.12 s |
| Pivot 正常范围 / 恢复带 / 硬边界 | 7.5 / 0.3 / 7.8 deg |
| Insertion 设计力 | 5 N |
| Insertion 目标/最大速度 | 6 / 8 mm/s |
| Insertion 时间常数 | 0.30 s |
| Insertion 正常范围 / 恢复带 / 硬边界 | ±15 / 1 / ±16 mm |
...
检查是否已经达到 `±15 mm` 正常相对插入行程边界。如果反馈轻微进入 `15--16 mm`
恢复带，控制器会禁止继续向外运动，但保留向内返回速度；只有超过 `±16 mm` 外层硬边界
才会进入 Fault。Pivot 使用相同机制：正常组合范围 `7.5 deg`、恢复带 `0.3 deg`、外层
硬边界 `7.8 deg`。
